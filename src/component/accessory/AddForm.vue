  ...
  <CommonModal v-bind="sourceModal" @cancle="$emit('cancle')" @serviceOk="serviceOk">
    ...
    >
      <ul v-if="pageList.length > 0" class="randar-list-wrap">
        <li v-for="item in pageList" :key="item.formId" class="randar-list" @click="selectRandarId = item.formId">
          <header class="randar-head">
            <div></div>
            <a-checkbox :checked="selectRandarId === item.formId"></a-checkbox>
          </header>
          <section class="randar-main">
            <div class="randar-main-content">
              <p>{{ item.name }}</p>
              <p class="aks-tips">{{ item.describe }}</p>
            </div>
...
import { customerFormList } from '@/api/customerConversion/formTable'
export default {
  name: 'AddForm',
  ...
  watch: {
    editData: {
      handler() {
        ...
        // } else if (this.editData?.formId) {
        //   this.selectRandarId = this.editData.formId
        // }
      },
      deep: true,
      immediate: true
  ...
  methods: {
    serviceOk() {
      if (this.selectRandarId) {
        const randar = this.pageList.find(item => item.formId === this.selectRandarId)
        randar.typeText = '自定义表单'
        randar.linkTitle = randar.name
        randar.desc = randar.describe
        randar.type = 'link'
        randar.key = 'visibileForm'
        randar.linktype = 'form_link'
        randar.coverUrl = {
